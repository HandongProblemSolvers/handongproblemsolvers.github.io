...
7. 또한 여기서 전봇대 간격이 k에서 k+1이 된 경우 항상 a의 값은 증가하며, c값은 감소한다.
8. 결국 이를 잘 관찰하면 전봇대가 움직여야하는 움직임 값은 **이차 방정식** 형태임을 알 수 있다.
9. 이러한 이차 방정식은 **삼분 탐색**을 이용하면 최소 값을 찾아 갈 수 있다.
10. 삼분 탐색은 [갓 라이](http://m.blog.naver.com/kks227/221432986308)님이 설명해 주실 것이다.
11. 삼분 탐색 컨셉과 코드는 아주 단순하다. 우리는 이 값 분포가 이차 방정식 형태임을 찾아내는 것이 중요한 컨셉임을 이해하면 된다.

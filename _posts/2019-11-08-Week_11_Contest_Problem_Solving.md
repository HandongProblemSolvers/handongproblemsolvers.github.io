...
12. (규칙 3) 만약 a > b라면, 규칙 1에 의하면 (a-b)만큼 증가하면 곧 (a+(a-b), b+(a-b)) = (2a-b, b)가 존재하며 이를 계속 반복하면, (a+(a-b)*n, b)가 존재한다.
...
5. 여기서 정렬이 된 상태면 (i < j)인 (ai,bi)와 (aj, bj)에 대해서 (ai <= aj)를 항상 만족한다. 또한 k<=i인 bj중 가장 큰값을 bmax라 하면, (bj <= bmax)를 만족하는 경우 이 버스노선을 포함됨을 알 수 있다.
